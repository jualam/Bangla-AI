...
def translate_text_en_bn(request):
    ...
    if not text:
        return Response({"message": "Invalid input", "success": False, "errors": "Text is required", "code": 400}, status=400)
    ...
    try:
        ...
        def generate():
            for chunk in chunks:
                response = client.chat.completions.create(
                    model="gpt-4-turbo",
                    messages=[
                        {"role": "system", "content": "তুমি একজন বাংলা সংবাদ অনুবাদক। সংবাদ ভাষায় অনুবাদ করবে।"},
                        {"role": "user", "content": f"এই ইংরেজি সংবাদটি বাংলায় অনুবাদ কর:\n{chunk}"}
                    ],
                    max_tokens=1000,
                    stream=True
                )
        ...
        return Response({"message": "Translation Failed", "success": False, "error": str(e), "code": 400}, status=400)
